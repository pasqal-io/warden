...
@pytest.mark.asyncio
async def test_empiric_cancel_and_scheduler_pick_race_no_double_claim(
    ...
    3. Assert the job never ends up with status CANCELED and a
       `scheduled_at` set: whichever side wins, the other must lose cleanly
...
class _HookedSession:
    """Session proxy running `hook` once, right after the first `execute`.

    Passed to `get_next_job`, it lands the hook in the candidate-pick ->
    claim window: the one read-then-write gap the scheduler still has.
    """
...
async def test_cancel_racing_scheduler_pick_is_not_claimed(
    ...
    async with async_session() as session:
        claimed = await scheduler.get_next_job(
            cast(AsyncSession, _HookedSession(session, cancel_mid_pick))
        )
...
@pytest.mark.asyncio
async def test_empiric_session_revoke_and_scheduler_pick_race_no_double_claim(
    client, app, serialized_sequence: str
):
